...
func TestPoolAcquireRelease(t *testing.T) {
	...
	// Acquire second (should get the other one, since it's LRU)
	p2 := pool.Acquire()
	if p2 == nil {
		t.Fatal("expected a proxy")
	}
	if p1.Host == p2.Host && p1.Port == p2.Port {
		t.Error("expected different proxy on second acquire")
	}
	...
	// Healthy count should be 1 now
	if pool.HealthyCount() != 1 {
		t.Errorf("expected 1 healthy, got %d", pool.HealthyCount())
	}

	// Acquire should only return the healthy one
	p3 := pool.Acquire()
	if p3 == nil {
		t.Fatal("expected a proxy")
	}
	if p3.Host == p1.Host && p3.Port == p1.Port {
		t.Error("should not acquire unhealthy proxy")
	}
}
...
func TestProxyString(t *testing.T) {
	...
	s := px.String()
	if s != "socks5://1.2.3.4:1080 (healthy, used 3 times)" {
